...
import com.alarmfm.radio.data.api.RadioBrowserApiManager
...
class RadioStationRepositoryImpl @Inject constructor(
    private val apiManager: RadioBrowserApiManager,
    ...
    override suspend fun getTopStations(): Result<List<RadioStation>> {
        return try {
            val stations = apiManager.executeWithFallback { api ->
                api.getPopularStations().map { it.toRadioStation() }
            }
    ...
    override suspend fun searchStations(query: String): Result<List<RadioStation>> {
        return try {
            val stations = apiManager.executeWithFallback { api ->
                api.searchStations(name = query).map { it.toRadioStation() }
            }
            Result.success(stations)
        } catch (e: Exception) {
            Result.failure(e)
    ...
    override suspend fun getStationsByTag(tag: String): Result<List<RadioStation>> {
        return try {
            val stations = apiManager.executeWithFallback { api ->
                api.getStationsByTag(tag).map { it.toRadioStation() }
            }
            Result.success(stations)
        } catch (e: Exception) {
            Result.failure(e)
    ...
    override suspend fun getStationsByCountry(country: String): Result<List<RadioStation>> {
        return try {
            val stations = apiManager.executeWithFallback { api ->
                api.getStationsByCountry(country).map { it.toRadioStation() }
            }
            Result.success(stations)
        } catch (e: Exception) {
            Result.failure(e)
    ...
    override suspend fun getStationsByLanguage(language: String): Result<List<RadioStation>> {
        return try {
            val stations = apiManager.executeWithFallback { api ->
                api.getStationsByLanguage(language).map { it.toRadioStation() }
            }
            Result.success(stations)
        } catch (e: Exception) {
            Result.failure(e)
